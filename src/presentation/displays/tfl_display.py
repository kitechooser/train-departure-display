...
class TflDisplay(BaseDisplay):
    ...
    def __init__(self, width: int = 256, height: int = 64, event_bus: Optional[EventBus] = None, display_id: str = "tfl"):
        ...
        Args:
            ...
            display_id: Unique identifier for this display (default: "tfl")
        """
        ...
        logger.info("TfL display initialized")
    ...
    def render(self) -> Image.Image:
        ...
        """
        logger.info("Rendering TfL display")
        ...
        if 'type' in event:
            if event['type'] == 'display_update':
                ...
                if 'station' in data and 'services' in data:
                    ...
                    if 'stations' in content:
                        logger.info(f"Handling display update with stations data")
                        # Update display content
                        self.update(content)
                    elif 'error' in content:
                        logger.info(f"Handling display error: {content['error']}")
                        self.update({'error': content['error']})
    ...
    def draw_services(self, services: List[Service]) -> None:
        ...
        for row, service in enumerate(services):
            ...
            time_text = TextComponent(service.status, style, None)
